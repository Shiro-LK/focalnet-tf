...
class FocalModulation(nn.Module):
    ...
    def forward(self, x):
        ...
        #print('pt modulation : ', ctx[0, :10, 0, 0], ctx.shape)   
        ctx_global = self.act(ctx.mean(2, keepdim=True).mean(3, keepdim=True))
        ctx_all = ctx_all + ctx_global*self.gates[:,self.focal_level:]
        #print("pt modulation :", ctx[0,:10,0,0])
        # normalize context
    ...
    def forward(self, x, H, W):
        for blk in self.blocks:
            ...
            if self.use_checkpoint:
                ...
                x = blk(x)
                #print("pt basic: ", x[0,0,:10])
        if self.downsample is not None:
            x = x.transpose(1, 2).reshape(x.shape[0], -1, H, W)
            #print("before downsample pt : ", x.shape)
            x, Ho, Wo = self.downsample(x)
            #print("before after pt : ", x.shape)
        else:
            Ho, Wo = H, W        
        return x, Ho, Wo
...
def load_focalnet(model_name,  num_classes=None, **kwargs):
    focal_levels=[2,2,2,2]
    focal_windows = [3, 3, 3, 3]
    use_conv_embed = False 
    use_postln = False 
...
if __name__ == '__main__':
    ...
    weights = torch.load("ckpt/focalnet_tiny_lrf.pth")
